...
export default function ThirdPartView({ sequenceId }: { sequenceId: number }) {
  ...
  const fetchQuestions = useCallback(async () => {
    setIsLoading(true);
    const result = await getSequenceQuestionsPart(sequenceId, 3);
    if (result.isSuccess) {
  ...
  const cleanOrder = useCallback(async () => {
    setIsLoading(true);
    const result = await cleanSequencePartOrderActions(sequenceId, 3);
    if (result.isSuccess) {
      fetchQuestions().then(() => {
        setIsLoading(false);
      });
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="text-sm text-gray-500">
        Trzecia część gry nie ma wymaganej liczby pytań, trwa puki nie zostanie
        jeden gracz - zwycięzca.
      </div>
      <Divider />
